...
public class Particle extends Quad {
	...
	public Particle(float[] colors, float posX, float posY) {
		...
		mAge = 0;
		 ...
		 //random double gives  a number between 0 and MAX_SPEED*2. decrement by MAX_SPEED in order to have the possibility to move with the maximum speed (along each axis) but at the same time be able to
		 // move in both directions, both positive and negative.

		mXv = (rndDbl(0, MAX_SPEED * 2) - MAX_SPEED);
		mYv = (rndDbl(0, MAX_SPEED * 2) - MAX_SPEED);

		 // Smoothing out the diagonal speed.By the Pythagorean theorem, x^2 + y^2 = d^2.
